...
class ScraperTestCase(unittest.TestCase):
    ...
    def test_get_data_from_row_empty(self):
        newsoup = BeautifulSoup(
            '<tr><td class="bbr-position"><span></span></td><td class="bbr-band"><span></span></td><td class="bbr-conductor"><span></span></td><td class "bbr-draw"><span></span></td></tr>',
            "html.parser",
        )
        data = scraper.get_data_from_row(newsoup)
        self.assertEqual(
            data,
            {"position": "", "band": "", "conductor": "", "draw": "", "region": ""},
        )
    ...
    def test_get_contest(self):
        self.assertEqual(
            scraper.get_contest(self.url),
            "National Championship of Great Britain (Championship Section Final)",
        )
...
class GetUrlTestCase(unittest.TestCase):
    ...
    def test_get_5_urls(self):
        target = [
            "https://www.brassbandresults.co.uk/contests/national-finals-championship-section/2023-10-21",
    ...
    def test_get_all_urls(self):
        soup = scraper.make_soup(
            "https://www.brassbandresults.co.uk/contests/national-finals-championship-section"
        )
